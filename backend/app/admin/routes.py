...
def suspend_chat_web(secret_path, chat_id):
    ...
    db.session.commit()
    return _flash_redirect(
        secret_path, f'chats/{chat_id}', 'ok',
...
def delete_message_web(secret_path, message_id):
    ...
    db.session.commit()
    return _flash_redirect(secret_path, 'messages', 'ok', 'پیام حذف شد')
